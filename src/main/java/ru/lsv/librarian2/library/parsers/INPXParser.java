...
public class INPXParser {
	 ...
	 * 
	 * @param inpxFileName
	 *            Имя inpx-файла для обработки
	 * @throws java.io.IOException
	 *             В случае проблем чтения из inpx-файла
	 */
	...
	public INPXParser(String inpxFileName) throws IOException {
		...
		for (Enumeration e = zip.entries(); e.hasMoreElements();) {
			...
			if ("inp".equals(FilenameUtils.getExtension(ze.getName()))) {
				// Значит это INP-файл
				// В связи с тем, что, к примеру, в Либрусеке оно может быть в
				// 1251 и в UTF-8 - придется
				// детектировать чарсет. К сожалению. Как определить "на лету" -
				// я что-то хз
				//
				// Пример использования из
				...
				//
				// Получили кодировку
				BufferedReader binp = new BufferedReader(new InputStreamReader(
				...
				while ((inpLine = binp.readLine()) != null) {
					try {
						...
						// В случае возникновения exception - просто ничего
						// никуда не добавится
					}
				}
			}
